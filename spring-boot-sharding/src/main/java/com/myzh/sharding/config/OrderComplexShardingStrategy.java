...
public class OrderComplexShardingStrategy implements ComplexKeysShardingAlgorithm {
	...
	public Collection<String> doSharding(Collection<String> availableTargetNames,
		...
		log.info("Tables: {}, preciseValue: {}", JSON.toJSONString(availableTargetNames),
				JSON.toJSONString(shardingValues));
		Collection<Timestamp> years = shardingValues.stream().filter(t -> t.getColumnName().equalsIgnoreCase(TIME))
				.map(o -> ((ListShardingValue<Timestamp>) o).getValues()).findFirst().orElse(null);
		Collection<Long> orders = shardingValues.stream().filter(t -> t.getColumnName().equalsIgnoreCase(ORDER_ID))
				.map(o -> ((ListShardingValue<Long>) o).getValues()).findFirst().orElse(null);

		if (availableTargetNames == null || availableTargetNames.isEmpty() || years == null || years.isEmpty()
